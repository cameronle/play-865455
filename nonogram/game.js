...
const saves=JSON.parse(localStorage.getItem('nonogram-saves')||'{}');
...
function updateCurrentLevelCard(){const card=$('levelGrid').children[puzzleIndex],saved=saves[bestKey()];if(!card||!saved)return;card.classList.add('in-progress');const detail=card.children[2];if(detail){const handled=saved.board.flat().filter(value=>value!==R.UNKNOWN).length;detail.textContent=`IN PROGRESS · ${Math.round(handled/(size*size)*100)}%`}}
function saveGame(){if(!puzzle||!board)return;const key=bestKey();saves[key]={board:board.map(row=>row.slice()),mistakes,mode,cursor:{...cursor},elapsed:elapsedSeconds(),updated:Date.now()};localStorage.setItem('nonogram-saves',JSON.stringify(saves));localStorage.setItem('nonogram-last-played',JSON.stringify({size,index:puzzleIndex,name:puzzle.name}));updateCurrentLevelCard()}
function loadSavedGame(){const saved=saves[bestKey()];if(!saved||!Array.isArray(saved.board)||saved.board.length!==size||saved.board.some(row=>!Array.isArray(row)||row.length!==size))return false;board=saved.board.map(row=>row.slice());mistakes=Number(saved.mistakes)||0;mode=saved.mode===R.MARKED?R.MARKED:R.FILLED;cursor=saved.cursor&&Number.isInteger(saved.cursor.r)&&Number.isInteger(saved.cursor.c)?saved.cursor:{r:0,c:0};startedAt=Date.now()-(Number(saved.elapsed)||0)*1000;return true}
function clearSavedGame(){if(!puzzle)return;delete saves[bestKey()];localStorage.setItem('nonogram-saves',JSON.stringify(saves));renderLevelGrid()}
...
function paint(r,c,value,fromDrag=false){
 ...
 render();if(mistakes>=3){finish(false);return}if(R.isSolved(board,puzzle.solution)){finish(true);return}saveGame();
}
...
function renderLevelGrid(){const list=R.listPuzzles(size),grid=$('levelGrid');grid.replaceChildren();list.forEach((item,index)=>{const key=`${size}-${item.name}`,done=completed.has(key),saved=saves[key],inProgress=!done&&saved&&Array.isArray(saved.board),card=document.createElement('button');card.type='button';card.className='level-card';card.classList.toggle('completed',done);card.classList.toggle('in-progress',Boolean(inProgress));card.classList.toggle('current',index===puzzleIndex);card.appendChild(previewNode(item));const title=document.createElement('strong');title.textContent=`${String(index+1).padStart(2,'0')} / ${item.name}`;card.appendChild(title);const detail=document.createElement('small');const handled=inProgress?saved.board.flat().filter(value=>value!==R.UNKNOWN).length:0;const percent=inProgress?Math.round(handled/(item.size*item.size)*100):0;detail.textContent=done?(bests[key]?`BEST ${formatTime(bests[key])}`:'COMPLETE'):inProgress?`IN PROGRESS · ${percent}%`:'NOT STARTED';card.appendChild(detail);const check=document.createElement('span');check.className='check';check.textContent='✓';card.appendChild(check);card.addEventListener('click',()=>{puzzleIndex=index;$('puzzleSelect').value=String(index);closeLevels();start()});grid.appendChild(card)});updateProgress()}
...
function undo(){const state=history.pop();if(!state)return;board=state.board;mistakes=state.mistakes;active=true;$('resultOverlay').classList.remove('show');render();saveGame()}
function hint(){if(!active)return;for(let r=0;r<size;r++)for(let c=0;c<size;c++){const correct=puzzle.solution[r][c]?R.FILLED:R.MARKED;if(board[r][c]!==correct){snapshot();board[r][c]=correct;cursor={r,c};render();if(R.isSolved(board,puzzle.solution))finish(true);else saveGame();return}}}
function resetCurrent(){clearSavedGame();start(true)}
...
try{const last=JSON.parse(localStorage.getItem('nonogram-last-played')||'null');if(last&&[5,10,15].includes(Number(last.size))){size=Number(last.size);puzzleIndex=Math.max(0,Number(last.index)||0);$('sizeSelect').value=String(size)}}catch{}size=Number($('sizeSelect').value);populatePuzzles();$('puzzleSelect').value=String(puzzleIndex);start();
})();
