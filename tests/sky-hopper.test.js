...
test('Doodle Hop has a complete mobile-first static entrypoint',()=>{const html=read('sky-hopper/index.html'),css=read('sky-hopper/style.css');assert.match(html,/viewport-fit=cover/);assert.match(html,/user-scalable=no/);for(const id of ['game','startButton','pauseButton','leftButton','rightButton','score','stars','best','height'])assert.match(html,new RegExp(`id="${id}"`));assert.match(html,/DOODLE HOP/);assert.match(css,/touch-action:none/);assert.match(css,/user-select:none/);});
...
test('stars are generated, collected, scored and persisted',()=>{const js=read('sky-hopper/game.js');assert.match(js,/function addStar/);assert.match(js,/function collectStars/);assert.match(js,/starCount\+\+/);assert.match(js,/doodleHopBestStars/);});
