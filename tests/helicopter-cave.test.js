...
test('long pressing the rise control cannot select text or open the touch callout',()=>{const css=read('helicopter-cave/style.css'),js=read('helicopter-cave/game.js');assert.match(css,/\.thrust\{[^}]*user-select:none/);assert.match(css,/\.thrust\{[^}]*-webkit-user-select:none/);assert.match(css,/\.thrust\{[^}]*-webkit-touch-callout:none/);assert.match(js,/selectstart/);assert.match(js,/contextmenu/);});
