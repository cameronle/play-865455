...
function solvable(rows,R){
  ...
  const queue=[{player:start.player,boxes:new Set(Object.keys(start.boxes))}],seen=new Set([encode(start.player,new Set(Object.keys(start.boxes)))]);
  const dirs=[[1,0],[-1,0],[0,1],[0,-1]];
  for(let head=0;head<queue.length&&head<500000;head++){
    const cur=queue[head];if([...cur.boxes].every(box=>goals.has(box)))return true;
    for(const [dx,dy] of dirs){
      const nx=cur.player.x+dx,ny=cur.player.y+dy,nk=key(nx,ny);
      if(nx<0||ny<0||nx>=start.width||ny>=start.height||walls.has(nk))continue;
      const boxes=new Set(cur.boxes);
      if(boxes.has(nk)){
        const bx=nx+dx,by=ny+dy,bk=key(bx,by);
        if(bx<0||by<0||bx>=start.width||by>=start.height||walls.has(bk)||boxes.has(bk))continue;
        boxes.delete(nk);boxes.add(bk)
      }
      const player={x:nx,y:ny},id=encode(player,boxes);
      if(!seen.has(id)){seen.add(id);queue.push({player,boxes})}
    }
  }
  return false;
}
...
test('every Sokoban level is structurally valid and all twenty levels are solver-verified',()=>{
  ...
  levels.forEach((rows,index)=>{
    ...
    assert.equal(solvable(rows,R),true,`level ${index+1} is not solvable`);
  });
});
