...
test('runner supports variable-height jump, coyote time, jump buffering, and fast fall',()=>{const js=read('endless-runner/game.js');assert.match(js,/JUMP_SPEED/);assert.match(js,/GRAVITY/);assert.match(js,/jumpHeld/);assert.match(js,/fastFall/);assert.match(js,/jumpBuffer/);assert.match(js,/coyoteTime/);assert.match(js,/function jump/)});
test('mobile controls put fast fall on the left and the primary jump action on the right',()=>{const html=read('endless-runner/index.html');assert.ok(html.indexOf('id="dropButton"')<html.indexOf('id="jumpButton"'));});
...
test('game includes varied stationery obstacles, gaps, coins and progressive speed',()=>{const js=read('endless-runner/game.js');for(const marker of ['spike','crate','pencil','ruler','ink','gap','coin','scrollSpeed'])assert.match(js,new RegExp(marker));});
