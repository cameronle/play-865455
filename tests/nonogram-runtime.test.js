...
function make(tag='div',id=''){
 const listeners={};
 const properties={};
 return{tag,id,textContent:'',innerHTML:'',value:id==='sizeSelect'?'10':'',disabled:false,dataset:{},style:{properties,setProperty(key,value){properties[key]=value}},children:[],attributes:{},classList:{values:new Set(),add(...x){x.forEach(v=>this.values.add(v))},remove(...x){x.forEach(v=>this.values.delete(v))},toggle(x,on){if(on)this.values.add(x);else this.values.delete(x)},contains(x){return this.values.has(x)}},appendChild(child){this.children.push(child);if(!this.value&&tag==='select')this.value=String(child.value)},replaceChildren(...children){this.children=children},querySelector(selector){return selector==='b'?this.children.find(child=>child.tag==='b')||null:null},addEventListener(type,fn){(listeners[type]??=[]).push(fn)},dispatch(type,event={}){for(const fn of listeners[type]||[])fn({preventDefault(){},target:this,pointerId:1,button:0,key:'',...event})},setAttribute(k,v){this.attributes[k]=v},closest(selector){return selector==='.cell'&&(this.classList.contains('cell')||String(this.className||'').split(/\s+/).includes('cell'))?this:null},setPointerCapture(){}};
}
...
test('TRY AGAIN clears a failed board, mistakes, timer state, and failed save',()=>{
 const ids=['board','rowClues','colClues','sizeSelect','puzzleSelect','modeFill','modeMark','undoButton','hintButton','newButton','resetButton','timer','mistakes','best','resultOverlay','resultTitle','resultText','resultButton','puzzleFrame','levelButton','progressSummary','levelOverlay','levelGrid','closeLevels','galleryProgress','revealName'];
 const elements=new Map(ids.map(id=>[id,make(id==='sizeSelect'||id==='puzzleSelect'?'select':'div',id)]));
 const reveal=elements.get('revealName');reveal.appendChild(make('span'));reveal.appendChild(make('b'));elements.get('sizeSelect').value='10';
 const storage={};
 const sandbox={window:{NonogramRules:rules(),matchMedia(){return{matches:true}}},document:{getElementById:id=>elements.get(id),createElement:tag=>make(tag),addEventListener(){},elementFromPoint(){return null}},localStorage:{getItem(key){return storage[key]??null},setItem(key,value){storage[key]=String(value)}},setInterval(){return 1},clearInterval(){},setTimeout(fn){fn()},Date,Math,JSON,console};
 vm.createContext(sandbox);vm.runInContext(fs.readFileSync('nonogram/game.js','utf8'),sandbox);
 const wrong=[0,1,2];
 for(const index of wrong){elements.get('board').dispatch('pointerdown',{target:elements.get('board').children[index]});elements.get('board').dispatch('pointerup')}
 assert.equal(elements.get('resultTitle').textContent,'PUZZLE FAILED');
 assert.equal(elements.get('mistakes').textContent,'3 / 3');
 assert.match(storage['nonogram-saves'],/10-ROCKET/);
 elements.get('resultButton').dispatch('click');
 assert.equal(elements.get('mistakes').textContent,'0 / 3');
 assert.equal(elements.get('timer').textContent,'00:00');
 assert.ok(elements.get('board').children.every(cell=>!cell.classList.contains('filled')&&!cell.classList.contains('marked')));
 assert.doesNotMatch(storage['nonogram-saves'],/10-ROCKET/);
 assert.equal(elements.get('resultOverlay').classList.contains('show'),false);
});
