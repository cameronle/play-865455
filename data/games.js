'use strict';

// Canonical metadata for the game launcher and generated documentation.
module.exports = [
  {
    "id": "2048",
    "path": "2048",
    "order": 1,
    "category": "puzzle",
    "name": {
      "zh": "2048",
      "en": "2048"
    },
    "description": {
      "zh": "滑动合并相同数字，冲击 2048 方块。",
      "en": "Join the numbers. Chase the tile."
    },
    "readme": {
      "title": "Classic 2048",
      "description": "本仓库自维护的纯 HTML5/CSS/JavaScript 2048，支持键盘、触摸、计分和本地最高分。"
    }
  },
  {
    "id": "shooter",
    "path": "shooter",
    "order": 2,
    "category": "arcade",
    "name": {
      "zh": "星空巡航",
      "en": "SKY PATROL"
    },
    "description": {
      "zh": "移动、自动发射、躲避敌机，经典飞行射击。",
      "en": "Move, fire, survive. A small original shooter."
    },
    "readme": {
      "title": "Sky Patrol",
      "description": "自维护的 HTML5 Canvas 街机飞行射击游戏，支持自动发射、关卡、生命、道具、音效和移动端触控。"
    }
  },
  {
    "id": "tetris",
    "path": "tetris",
    "order": 3,
    "category": "puzzle",
    "name": {
      "zh": "俄罗斯方块",
      "en": "TETRIS"
    },
    "description": {
      "zh": "经典下落方块，消除整行，冲击更高分。",
      "en": "Classic falling blocks. Clear lines. Keep going."
    },
    "readme": {
      "title": "Classic Tetris",
      "description": "自维护的纯 HTML5 Canvas 俄罗斯方块，支持键盘、完整触控、计分、等级、下一个方块和本地最高分。"
    }
  },
  {
    "id": "snake",
    "path": "snake",
    "order": 4,
    "category": "arcade",
    "name": {
      "zh": "毛毛虫花园",
      "en": "WORM & APPLE"
    },
    "description": {
      "zh": "操控贪吃的小毛毛虫收集苹果和浆果，越吃越长。",
      "en": "Guide a hungry little worm through the garden and grow longer."
    },
    "readme": {
      "title": "Worm & Apple",
      "description": "自维护的卡通涂鸦贪食蛇，支持苹果、浆果和叶子收集、加速、穿墙、键盘与触控操作。"
    }
  },
  {
    "id": "breakout",
    "path": "breakout",
    "order": 5,
    "category": "arcade",
    "name": {
      "zh": "打砖块",
      "en": "BREAKOUT"
    },
    "description": {
      "zh": "弹射小球击碎砖块，考验反应与走位。",
      "en": "Break the wall. Keep the ball alive."
    },
    "readme": {
      "title": "Breakout",
      "description": "自维护的纯 HTML5 Canvas 打砖块，支持反弹、关卡、生命、本地最高分、手机拖动和一键开始发球。"
    }
  },
  {
    "id": "minesweeper",
    "path": "minesweeper",
    "order": 6,
    "category": "puzzle",
    "name": {
      "zh": "经典扫雷",
      "en": "MINESWEEPER"
    },
    "description": {
      "zh": "推算数字排查雷区，20 关逐级挑战。",
      "en": "Read the numbers. Avoid the mines."
    },
    "readme": {
      "title": "Minesweeper",
      "description": "自维护的经典扫雷，包含 20 个逐关解锁的关卡，支持首次落子保护、点击、右键或长按插旗、计时和每关本地最佳时间。"
    }
  },
  {
    "id": "space-invaders",
    "path": "space-invaders",
    "order": 7,
    "category": "arcade",
    "name": {
      "zh": "太空侵略者",
      "en": "ALIEN FORMATION"
    },
    "description": {
      "zh": "迎击步步逼近的外星阵列，守卫防线。",
      "en": "Defend the line against descending aliens."
    },
    "readme": {
      "title": "Alien Formation",
      "description": "自维护的经典固定阵型射击游戏，支持自动发射、左右移动、波次、受击保护、手机触控和本地最高分。"
    }
  },
  {
    "id": "maze",
    "path": "maze",
    "order": 8,
    "category": "arcade",
    "name": {
      "zh": "霓虹迷宫",
      "en": "NEON MAZE"
    },
    "description": {
      "zh": "吃完所有光点，巧妙避开巡逻幽灵。",
      "en": "Eat every light. Avoid the hunters."
    },
    "readme": {
      "title": "Neon Maze",
      "description": "原创迷宫吃点游戏，支持寻路敌人、关卡、生命、本地最高分、手机滑动和触屏方向键。"
    }
  },
  {
    "id": "gomoku",
    "path": "gomoku",
    "order": 9,
    "category": "strategy",
    "name": {
      "zh": "五子棋",
      "en": "GOMOKU"
    },
    "description": {
      "zh": "执黑先行，在电脑拦截前率先连成五子。",
      "en": "Play as Black. Connect five before the computer does."
    },
    "readme": {
      "title": "Gomoku",
      "description": "自维护的 15×15 人机五子棋，支持三档难度、撤销、鼠标与触控操作、最后落子标记和本地战绩。"
    }
  },
  {
    "id": "pong",
    "path": "pong",
    "order": 10,
    "category": "arcade",
    "name": {
      "zh": "弹球对决",
      "en": "PONG"
    },
    "description": {
      "zh": "滑动球拍精准截击，与电脑一较高下。",
      "en": "Deflect the ball. Outsmart the machine."
    },
    "readme": {
      "title": "Pong",
      "description": "自维护的人机乒乓球游戏，支持指针拖动、目标比分和本地战绩。"
    }
  },
  {
    "id": "sokoban",
    "path": "sokoban",
    "order": 11,
    "category": "puzzle",
    "name": {
      "zh": "小熊搬果篮",
      "en": "BEAR & BOXES"
    },
    "description": {
      "zh": "帮小熊把果篮推回野餐点，规划路线完成 20 个森林关卡。",
      "en": "Help a little bear push fruit crates back to the picnic baskets."
    },
    "readme": {
      "title": "Bear & Boxes",
      "description": "自维护的卡通涂鸦推箱子游戏，包含 20 个原创关卡、撤销、重置、键盘、滑动和触控操作。"
    }
  },
  {
    "id": "crosswalk",
    "path": "crosswalk",
    "order": 12,
    "category": "arcade",
    "name": {
      "zh": "小鸭过街",
      "en": "TINY CROSSING"
    },
    "description": {
      "zh": "护送小鸭子穿过车流、红绿灯和施工路段，平安回到家。",
      "en": "Guide a little duck through traffic, signals, and roadworks to get home."
    },
    "readme": {
      "title": "Tiny Crossing",
      "description": "自维护的卡通涂鸦车流躲避街机，包含 20 个逐关解锁关卡、信号节奏、路线选择、移动安全区，支持生命、键盘、滑动和触控操作。"
    }
  },
  {
    "id": "simon",
    "path": "simon",
    "order": 13,
    "category": "memory",
    "name": {
      "zh": "怪兽乐队",
      "en": "MONSTER BAND"
    },
    "description": {
      "zh": "记住四只小怪兽的节奏，按正确顺序让乐队继续演奏。",
      "en": "Remember the monster beat and play it back in the right order."
    },
    "readme": {
      "title": "Monster Band",
      "description": "自维护的卡通涂鸦音序记忆游戏，包含四只小怪兽、音效、严格模式和本地最高分。"
    }
  },
  {
    "id": "sudoku",
    "path": "sudoku",
    "order": 14,
    "category": "puzzle",
    "name": {
      "zh": "数独",
      "en": "SUDOKU"
    },
    "description": {
      "zh": "在九宫格填入数字，满足行列唯一解。",
      "en": "Place the digits. Complete every row, column, and box."
    },
    "readme": {
      "title": "Sudoku",
      "description": "自维护的数独游戏，支持唯一解题目、三档难度、笔记、撤销、提示、错误计数、计时和本地最佳时间。"
    }
  },
  {
    "id": "lunar-lander",
    "path": "lunar-lander",
    "order": 15,
    "category": "simulation",
    "name": {
      "zh": "月球着陆器",
      "en": "LUNAR LANDER"
    },
    "description": {
      "zh": "控制推进与姿态，在有限燃料下平稳着陆。",
      "en": "Manage angle, velocity, and fuel for a safe landing."
    },
    "readme": {
      "title": "Lunar Lander",
      "description": "自维护的月球着陆物理游戏，支持随机地形、有限燃料、递增重力、完整触控和本地最高分。"
    }
  },
  {
    "id": "connect-four",
    "path": "connect-four",
    "order": 16,
    "category": "strategy",
    "name": {
      "zh": "四子棋",
      "en": "CONNECT FOUR"
    },
    "description": {
      "zh": "轮流落子，抢先在横竖斜任意方向连成四子。",
      "en": "Drop four in a row before the computer does."
    },
    "readme": {
      "title": "Connect Four",
      "description": "自维护的人机四子棋，支持三档 AI、先手选择、撤销、触控操作和本地战绩。"
    }
  },
  {
    "id": "sky-hopper",
    "path": "sky-hopper",
    "order": 17,
    "category": "arcade",
    "name": {
      "zh": "涂鸦弹跳",
      "en": "DOODLE HOP!"
    },
    "description": {
      "zh": "在手绘平台间自动弹跳，收集星星，继续向上冒险。",
      "en": "Bounce through a hand-drawn sky, collect stars, and climb higher."
    },
    "readme": {
      "title": "Doodle Hop!",
      "description": "自维护的卡通涂鸦纵向平台跳跃游戏，支持自动弹跳、星星收集、五种平台、键盘与触控操作和本地最高分。"
    }
  },
  {
    "id": "helicopter-cave",
    "path": "helicopter-cave",
    "order": 18,
    "category": "arcade",
    "name": {
      "zh": "直升机洞穴",
      "en": "HELICOPTER CAVE"
    },
    "description": {
      "zh": "按住上升松开下落，穿行狭窄起伏的地下洞穴。",
      "en": "Hold to rise, release to fall, and survive the cave."
    },
    "readme": {
      "title": "Helicopter Cave",
      "description": "自维护的单键洞穴飞行游戏，支持程序生成的可通行地形、障碍、速度递增、键盘与触控操作和本地最远距离。"
    }
  },
  {
    "id": "endless-runner",
    "path": "endless-runner",
    "order": 19,
    "category": "arcade",
    "name": {
      "zh": "无尽奔跑",
      "en": "ENDLESS RUNNER"
    },
    "description": {
      "zh": "跨越尖刺与坑洞、收集金币，跟上加速节奏。",
      "en": "Jump, collect, and keep pace with the accelerating road."
    },
    "readme": {
      "title": "Endless Runner",
      "description": "自维护的横版跑酷游戏，支持高低跳、快速落地、按速度生成的公平障碍、坑洞、金币、触控操作和本地纪录。"
    }
  },
  {
    "id": "solitaire",
    "path": "solitaire",
    "order": 20,
    "category": "card",
    "name": {
      "zh": "纸牌接龙",
      "en": "SOLITAIRE"
    },
    "description": {
      "zh": "红黑交替排布纸牌，将四色花色按序归位。",
      "en": "Build down in alternating colors. Send every suit home."
    },
    "readme": {
      "title": "Solitaire",
      "description": "自维护的经典 Klondike 接龙，支持点选移动、双击自动收牌、撤销、提示和本地战绩。"
    }
  },
  {
    "id": "bubble-shooter",
    "path": "bubble-shooter",
    "order": 21,
    "category": "puzzle",
    "name": {
      "zh": "泡泡花园",
      "en": "BUBBLE GARDEN"
    },
    "description": {
      "zh": "瞄准发射彩色花朵泡泡，让相同颜色的花朵一起绽放。",
      "en": "Aim colorful flower bubbles and make matching blossoms bloom together."
    },
    "readme": {
      "title": "Bubble Garden",
      "description": "自维护的卡通涂鸦泡泡射手，支持触屏瞄准、墙壁反弹、同色消除、悬空掉落和本地最高分。"
    }
  },
  {
    "id": "nonogram",
    "path": "nonogram",
    "order": 22,
    "category": "puzzle",
    "name": {
      "zh": "数织",
      "en": "NONOGRAM"
    },
    "description": {
      "zh": "根据行列数字推算填充，揭开隐藏像素图案。",
      "en": "Read the clues. Reveal the hidden pixel picture."
    },
    "readme": {
      "title": "Nonogram",
      "description": "自维护的数织像素谜题，包含 5×5、10×10 和 15×15 共 45 关，支持填充、标记、拖动绘制、撤销、提示、计时和本地最佳时间。"
    }
  },
  {
    "id": "flow",
    "path": "flow",
    "order": 23,
    "category": "puzzle",
    "name": {
      "zh": "连线解谜",
      "en": "FLOW FREE"
    },
    "description": {
      "zh": "连接相同颜色管道，无交叉铺满整个棋盘。",
      "en": "Connect matching colors. Fill the whole board."
    },
    "readme": {
      "title": "Flow Free",
      "description": "自维护的连线填格解谜游戏，包含 4 种尺寸共 40 关，支持触摸拖动连线、全盘覆盖验证、关卡选择与最佳时间。"
    }
  },
  {
    "id": "bridges",
    "path": "bridges",
    "order": 24,
    "category": "puzzle",
    "name": {
      "zh": "数桥",
      "en": "BRIDGES"
    },
    "description": {
      "zh": "在岛屿间架设指定数量桥梁，连通完整网络。",
      "en": "Connect the islands. Build a single network."
    },
    "readme": {
      "title": "Bridges",
      "description": "自维护的数桥解谜游戏，包含 3 种尺寸共 30 关，支持岛屿架桥、并查集连通性验证、撤销与关卡选择。"
    }
  },
  {
    "id": "sliding-puzzle",
    "path": "sliding-puzzle",
    "order": 25,
    "category": "puzzle",
    "name": {
      "zh": "数字华容道",
      "en": "15-PUZZLE"
    },
    "description": {
      "zh": "滑动方块归位，将乱序数字按顺序排列整齐。",
      "en": "Slide tiles into the empty space. Put numbers in order."
    },
    "readme": {
      "title": "15-Puzzle",
      "description": "自维护的数字华容道，支持 3×3、4×4、5×5 三种尺寸、可解性乱序生成、整行整列滑动、键盘操作与本地最佳纪录。"
    }
  },
  {
    "id": "color-bounce",
    "path": "color-bounce",
    "order": 26,
    "category": "arcade",
    "name": {
      "zh": "色彩跳跃",
      "en": "COLOR SWITCH"
    },
    "description": {
      "zh": "控制小球穿过相同颜色的几何障碍，节奏跃动。",
      "en": "Bounce through matching colors. Avoid wrong hues."
    },
    "readme": {
      "title": "Color Switch",
      "description": "自维护的色彩节拍弹跳街机，支持颜色匹配穿越、旋转几何体碰撞、换色道具与本地最高分。"
    }
  },
  {
    "id": "flappy",
    "path": "flappy",
    "order": 27,
    "category": "arcade",
    "name": {
      "zh": "小鸟邮差",
      "en": "BIRDIE POST"
    },
    "description": {
      "zh": "操控背着邮包的小鸟穿过云朵，安全送出每一封信。",
      "en": "Guide a tiny mail bird through cloud gaps and deliver every letter."
    },
    "readme": {
      "title": "Birdie Post",
      "description": "自维护的卡通涂鸦飞行街机，支持单键扑翼、云朵障碍、暂停、移动端触控和本地最高分。"
    }
  }
];
