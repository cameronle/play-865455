...
const LEVELS=[
  {
    ...
    platforms:[
      ...
      moving(905,380,116,55,1.35),moving(2540,360,105,62,1.1),
  ...
  },
  {
    ...
    platforms:[
      ...
      moving(690,372,105,70,1.45),moving(1335,345,110,75,1.25),moving(2100,365,100,85,1.5),moving(2790,365,110,80,1.4),moving(3510,335,105,65,1.3),
  ...
  },
  {
    ...
    platforms:[
      ...
      moving(770,370,110,80,1.55),moving(1470,355,105,75,1.5),moving(2240,360,100,80,1.65),moving(2960,365,105,90,1.55),moving(3750,350,105,80,1.7),moving(4480,350,100,85,1.65),
...
function drawPlayer(p){if(!player||player.inv>0&&Math.floor(player.inv*14)%2===0)return;const x=player.x+15-cameraX,y=player.y+21;ctx.save();ctx.translate(x,y);ctx.scale(player.face<0?-1:1,1);ctx.rotate(Math.sin(player.anim)*.035);ctx.strokeStyle=p.ink;ctx.lineWidth=2.5;ctx.lineJoin='round';ctx.lineCap='round';ctx.fillStyle=player.powered?p.mint:p.coral;ctx.beginPath();ctx.arc(0,-11,18,Math.PI,Math.PI*2);ctx.lineTo(18,-7);ctx.quadraticCurveTo(0,2,-18,-7);ctx.closePath();ctx.fill();ctx.stroke();ctx.fillStyle=p.paper;ctx.beginPath();ctx.arc(-8,-12,4,0,Math.PI*2);ctx.arc(7,-15,4.5,0,Math.PI*2);ctx.fill();ctx.stroke();ctx.fillStyle=player.powered?p.mint:p.blue;roundedRect(-13,-7,26,25,7);ctx.fill();ctx.stroke();ctx.fillStyle=p.yellow;ctx.fillRect(-14,-4,28,7);ctx.strokeRect(-14,-4,28,7);ctx.fillStyle=p.ink;ctx.beginPath();ctx.arc(-6,-19,2,0,Math.PI*2);ctx.arc(6,-19,2,0,Math.PI*2);ctx.fill();ctx.beginPath();ctx.arc(0,-12,6,0,Math.PI);ctx.stroke();ctx.strokeStyle=p.ink;ctx.lineWidth=4;ctx.beginPath();ctx.moveTo(-10,17);ctx.lineTo(-15,24);ctx.moveTo(10,17);ctx.lineTo(15,24);ctx.stroke();ctx.restore();}
