...
function hurtPlayer(){if(state!=='playing'||player.inv>0)return;if(player.powered){player.powered=false;player.inv=1.3;player.vy=-390;burst(player.x+15,player.y+20,palette().yellow,16);return;}lives--;setPlayerSpawn();player.inv=1.4;updateHud();if(lives<=0){gameOver();return;}}
