...
function start(){setup();state='play';$('overlay').classList.add('hide')}
...
function movePlayerButton(dir){const button=$(dir==='left'?'leftButton':'rightButton'),on=event=>{event.preventDefault();keys.add(dir)},off=event=>{event.preventDefault();keys.delete(dir)};button.addEventListener('pointerdown',on);['pointerup','pointercancel','pointerleave'].forEach(type=>button.addEventListener(type,off))}
...
window.onkeydown=event=>{keys.add(event.key);if(event.key===' '||event.key==='Spacebar'){event.preventDefault();startAndFire()}if(event.key==='p'||event.key==='P')pause()};window.onkeyup=event=>keys.delete(event.key);
